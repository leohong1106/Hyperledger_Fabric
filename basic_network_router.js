...
router.get('/query', async (req, res, next) => {
    ...
    const userExists = await wallet.exists('user1');
        ...
        const contract = network.getContract('sb');
...
router.post('/getHistory', async (req, res, next) => {
    try{
    console.log("history for : ", req.body.user);
    const userExists = await wallet.exists('user1');
        if (!userExists) {
            console.log('An identity for the user "user1" does not exist in the wallet');
            await res.json({'msg':'연결부터 해주세요'});
        ...
        var result = await contract.evaluateTransaction('getHistoryForMarble', `${req.body.user}`);
        console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
        res.json({'msg':result.toString()});
    }catch(e){
        console.log(e);
